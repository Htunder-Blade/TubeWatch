...
当前支持读取公开 YouTube 频道最近视频、用 SQLite 识别新增，并由用户显式把待处理视频交给已安装的 TubeScribe。首次检查时，抓取范围内的全部视频都视为新增；`check` 不会自动下载字幕。当前既可直接提供 `@wangzhian` 形式的频道 handle，也接受常见的 `/@handle`、`/channel/...`、`/c/...` 和 `/user/...` 完整频道 URL。
