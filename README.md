...
当前支持读取公开 YouTube 频道最近视频或指定播放列表、用 SQLite 识别新增，并由用户显式把待处理视频交给已安装的 TubeScribe。首次检查时，抓取范围内的全部视频都视为新增；`check` 不会自动下载字幕。频道既可直接提供 `@wangzhian` 形式的 handle，也接受常见的 `/@handle`、`/channel/...`、`/c/...` 和 `/user/...` 完整 URL。播放列表接受标准的 `/playlist?list=...` 完整 URL。
...
打开 `tests/notebooks/channel_tester.ipynb`，在配置单元格中填写必填的 `channel_handle`；需要附加监控播放列表时再填写选填的 `playlist_url`。Tester 会通过正式 CLI 先检查频道，再单独检查已配置的播放列表，并对每个来源连续检查两次以验证 SQLite 去重。它通过当前 Kernel 的 `sys.executable` 启动 `python -m tubewatch` 子进程，不直接导入项目源码。数据库会跨 Notebook 运行保留，因此以后运行时第一次检查也可能是零条新增。TubeWatch 必须先 editable 安装在这个 Python 环境中。不要保存包含私人 URL、Cookie 或运行输出的 Notebook。
